...
class pe_features():
    ...
    def get_count_suspicious_sections(self,pe):
        ...
        for section in pe.sections:
            # Decode section.Name from bytes to string, and then split
            section_name = section.Name.decode('utf-8').split('\x00')[0]
            tmp.append(section_name)
    ...
    def get_file_bytes_size(self,filepath):
        f = open(filepath, "rb")
        byteArr = f.read()  # No need to use ord() since bytes are already integers
        f.close()
